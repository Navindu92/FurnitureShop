...
{
    public partial class FrmPurchaseReturn : FrmBaseTransation
    {
        ///<summary>
        ...
        public override void FormLoad()
        {
            try
            {
                ...
                dgvPurchaseReturn.AutoGenerateColumns = false;
        ...
        public override void Initialize()
        {
            try
            {
                ...
                dgvPurchaseReturn.DataSource = null;
                dgvPurchaseReturn.DataSource = purchaseTempList;
                dgvPurchaseReturn.Refresh();
                ...
                isRecallGRN = false;
                Common.ReadOnlyTextBox(false, txtItemCode, txtItemName, txtCostPrice, txtSellingPrice);
        ...
        private void RecallGRN()
        {
            try
            {
                PurchaseService purchaseService = new PurchaseService();
                PurchaseMain purchaseMainReturn = new PurchaseMain();

                purchaseMainReturn = purchaseService.GetPurchaseReturnByGRNNo(cmbGRNNo.Text.Trim());

                if (purchaseMainReturn != null)
                {
                    SupplierService customerService = new SupplierService();
                    supplier = customerService.GetActiveSupplierByID(purchaseMainReturn.SupplierID);
                    if (supplier != null)
                    {
                    ...
                    recallDocumentID = purchaseMainReturn.PurchaseMainID;
                    recallDocumentDocumentID = FormInfoService.GetFormInfoByName("FrmGoodsReceivedNote").DocumentID;

                    purchaseTempList = new List<PurchaseTemp>();
                    purchaseTempList = purchaseService.GetPurchaseReturnSubByPurchaseMainID(purchaseMainReturn.PurchaseMainID, purchaseMainReturn.LocationID);
                    dgvPurchaseReturn.DataSource = purchaseTempList.ToList();
                    dgvPurchaseReturn.Refresh();

                    txtSubTotalDis.Text = Common.ConvertToStringCurrancy(purchaseMainReturn.DiscountPercentage.ToString());

                    Summarize();
                    EnableHeaderAndFooter(false);
                    isRecallGRN = true;
                }
            }
            catch (Exception ex)
        ...
        private void LoadDocumentNumbers()
        {
            try
            {
                ...
                cmbGRNNo.DataSource = null;
                cmbGRNNo.Items.Clear();
                cmbGRNNo.Refresh();
                Common.SetAutoComplete(cmbGRNNo, purchaseService.GetGRNNumbersForReturn());
        ...
        private void EnableHeaderAndFooter(bool enable)
        {
            try
            {
                Common.EnableTextBox(enable, txtSupplierCode, txtSupplierDescription, txtReferenceNo, txtSubTotalDis);
                Common.EnableComboBox(enable, cmbGRNNo);
            }
            catch (Exception ex)
            {
                LogWritter.WriteErrorLog(this.Name, MethodInfo.GetCurrentMethod().ToString(), ex.GetType().ToString(), ex.Message.ToString());
        ...
        private void dgvPurchaseReturn_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (dgvPurchaseReturn.RowCount > 0)
                {
                    if (dgvPurchaseReturn["ItemCode", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString() == string.Empty) { return; }
                    LoadItem(true, dgvPurchaseReturn["ItemCode", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());

                    txtCostPrice.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["CostPrice", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtSellingPrice.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["SellingPrice", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtMargin.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["Margin", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtQty.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["Qty", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtFreeQty.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["FreeQty", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtItemDis.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["DiscountPercentage", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtAmount.Text = Common.ConvertToStringCurrancy(dgvPurchaseReturn["Amount", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                    txtQty.SelectAll();
                    txtQty.Focus();

                    if (isRecallGRN)
                    {
                        Common.ReadOnlyTextBox(true, txtItemCode, txtItemName, txtCostPrice, txtSellingPrice);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void SavePurchaseReturn(bool isPrintPurchaseReturn = true)
        {
            try
            {
                Summarize();
                ...
                purchaseMain.DocumentDate = dtpDocumentDate.Value.Date;
                purchaseMain.LocationID = Common.LoggedLocationID;
                purchaseMain.SupplierID = supplier.SupplierID;
                purchaseMain.ReferenceNo = txtReferenceNo.Text.Trim();

                purchaseMain.TotalAmount = Common.ConvertStringToDecimal(txtTotalAmount.Text.Trim());
                if (txtSubTotalDis.Text != string.Empty) { purchaseMain.DiscountPercentage = Common.ConvertStringToDecimal(txtSubTotalDis.Text.Trim()); }
                if (txtDiscountAmount.Text != string.Empty) { purchaseMain.DiscountAmount = Common.ConvertStringToDecimal(txtDiscountAmount.Text.Trim()); }
                purchaseMain.NetAmount = Common.ConvertStringToDecimal(txtNetAmount.Text.Trim());

                PurchaseService purchaseService = new PurchaseService();

                if (isRecallGRN)
                {
                    PurchaseMain purchaseMainGRN = new PurchaseMain();
                    purchaseMainGRN = purchaseService.GetPurchaseReturnByGRNNo(cmbGRNNo.Text.Trim());

                    if (purchaseMainGRN != null)
                    {
                        purchaseMain.ReferenceDocumentID = purchaseMainGRN.PurchaseMainID;
                        purchaseMain.ReferenceDocumentDocumentID = purchaseMainGRN.DocumentID;
                    }
                }

                if (purchaseService.SavePurchaseReturn(formInfo, purchaseMain, purchaseTempList, out documentNo))
                {
                    Clear();
                    GeneratePurchaseReturnDocument();
                }
            }
            catch (Exception ex)
            {
        ...
        public void ValidateStockForGRN()
        {
            PurchaseService purchaseService = new PurchaseService();
        }
        private void UpdateGrid(PurchaseTemp purchaseTemp)
        {
            try
            {
                ...
                if (dgvPurchaseReturn.RowCount > 0)
                {
                    ...
                    dgvPurchaseReturn.Rows[dgvPurchaseReturn.RowCount - 1].Selected = true;
                    dgvPurchaseReturn.FirstDisplayedScrollingRowIndex = dgvPurchaseReturn.RowCount - 1;
        ...
        private void GeneratePurchaseReturnDocument()
        {
            try
            {
            ...
            try
            {
                if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dgvPurchaseReturn.Rows.Count > 0)
                    {
                        ...
                        LoadItem(true, dgvPurchaseReturn["ItemCode", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                        ClearLine();
                        if (SysMessage.ShowMessage(SysMessage.MessageAction.Delete, SysMessage.MessageType.Question, this.Text, dgvPurchaseReturn["ItemCode", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString() + " - " + dgvPurchaseReturn["ItemName", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString()).Equals(DialogResult.No)) { return; };
                        if (item != null)
                        {
                            purchaseTemp = new PurchaseTemp();
                            purchaseTemp.ItemID = item.ItemID;
                            purchaseTemp.CostPrice = Common.ConvertStringToDecimal(dgvPurchaseReturn["CostPrice", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                            purchaseTemp.SellingPrice = Common.ConvertStringToDecimal(dgvPurchaseReturn["SellingPrice", dgvPurchaseReturn.CurrentCell.RowIndex].Value.ToString());
                        }
        ...
        private void txtQty_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (isRecallGRN)
                {
                    ...
                    if (item != null)
                    {
                        decimal grnQty = 0, freeGrnQty = 0;

                        PurchaseService purchaseService = new PurchaseService();

                        purchaseService.GetGRNQtyByDocumentNoAndProductID(cmbGRNNo.Text.Trim(), item.ItemID, out grnQty, out freeGrnQty);

                        if (Common.ConvertStringToDecimal(txtQty.Text.Trim()) > grnQty)
                        {
                            SysMessage.ShowMessage(SysMessage.MessageAction.General, SysMessage.MessageType.Information, this.Text, "Return Qty Should be Lesser Than GRN Qty");
                            txtQty.Focus();
                        }
                    }
                }
            ...
            try
            {
                if (isRecallGRN)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        if (Common.ConvertStringToDecimal(txtFreeQty.Text.Trim()) > freeGrnQty)
                        {
                            SysMessage.ShowMessage(SysMessage.MessageAction.General, SysMessage.MessageType.Information, this.Text, "Return Free Qty Should be Lesser Than GRN Free Qty");
                            txtQty.Focus();
                        }
                    }
                }
